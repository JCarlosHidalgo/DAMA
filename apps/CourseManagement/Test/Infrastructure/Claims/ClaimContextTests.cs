...
public class ClaimContextTests
{
    ...
    {
        var expected = new DateTime(2030, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        long unixSeconds = new DateTimeOffset(expected).ToUnixTimeSeconds();
        GivenClaims(new Claim("subscription_expires_at", unixSeconds.ToString()));
        var claimContext = new ClaimContext(_httpContextAccessor.Object);
    ...
    {
        GivenClaims(new Claim("subscription_expires_at", "1893456000"));
        var claimContext = new ClaimContext(_httpContextAccessor.Object);
