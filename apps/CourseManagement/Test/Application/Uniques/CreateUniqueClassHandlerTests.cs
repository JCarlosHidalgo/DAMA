...
public class CreateUniqueClassHandlerTests
{
    ...
    public async Task Handle_WhenAnyTeacherHasOverlap_ReturnsTeacherConflictAndSkipsCoordinator()
    {
        ...
        uniqueClassDao
            .Setup(dao => dao.HasOverlapForTeacherAsync(TenantId, teacherId, payload.Date, payload.StartTime, payload.EndTime, null))
            .ReturnsAsync(true);

        CreateUniqueClassResult result = await handler.Handle(new CreateUniqueClassCommand(payload));

        Assert.That(result, Is.InstanceOf<CreateUniqueClassResult.TeacherConflict>());
        var conflict = (CreateUniqueClassResult.TeacherConflict)result;
        Assert.Multiple(() =>
        {
    ...
    public async Task Handle_WhenCoordinatorReturnsCreated_ReturnsCreatedWithMappedDto()
    {
        ...
        var built = new UniqueClass { Id = Guid.NewGuid(), TenantId = TenantId };
        ...
        uniqueClassDao
            .Setup(dao => dao.HasOverlapForTeacherAsync(TenantId, teacherId, payload.Date, payload.StartTime, payload.EndTime, null))
            .ReturnsAsync(false);
        classBuilder.Setup(builder => builder.BuildUniqueClass(TenantId, payload.CourseId, payload, teachers)).Returns(built);
        coordinator
    ...
    public async Task Handle_WhenCoordinatorReturnsReplayed_ReturnsReplayedFromIdempotencyWithMappedPrior()
    {
        ...
        var built = new UniqueClass { Id = Guid.NewGuid(), TenantId = TenantId };
        var prior = new UniqueClass { Id = Guid.NewGuid(), TenantId = TenantId };
