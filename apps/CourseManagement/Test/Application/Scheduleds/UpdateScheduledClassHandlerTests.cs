...
public class UpdateScheduledClassHandlerTests
{
    ...
    [Test]
    public async Task Handle_WhenTeacherHasOverlap_ReturnsTeacherConflictWithoutBeginningTransaction()
    {
        ...
        scheduledClassDao
            .Setup(dao => dao.HasOverlapForTeacherAsync(TenantId, conflictTeacherId, payload.DayOfWeekIndex, payload.StartTime, payload.EndTime, ScheduledClassId))
            .ReturnsAsync(true);
        ...
        Assert.Multiple(() =>
        {
            Assert.That(result, Is.InstanceOf<UpdateScheduledClassResult.TeacherConflict>());
            Assert.That(((UpdateScheduledClassResult.TeacherConflict)result).TeacherName, Is.EqualTo("Profesor"));
        });
    ...
    public async Task Handle_WhenUpdateReturnsFalse_ReturnsNotFoundAndDoesNotCommit()
    {
        ...
        mapper.Setup(map => map.Map<List<ClassTeacherDto>, List<ClassTeacher>>(teacherDtos)).Returns(teacherEntities);
        scheduledClassDao
            .Setup(dao => dao.HasOverlapForTeacherAsync(TenantId, teacherId, payload.DayOfWeekIndex, payload.StartTime, payload.EndTime, ScheduledClassId))
            .ReturnsAsync(false);
        scheduledClassDao
    ...
    public async Task Handle_WhenUpdateSucceeds_ReplacesTeachersCommitsAndReturnsUpdated()
    {
        ...
        mapper.Setup(map => map.Map<List<ClassTeacherDto>, List<ClassTeacher>>(teacherDtos)).Returns(teacherEntities);
        scheduledClassDao
            .Setup(dao => dao.HasOverlapForTeacherAsync(TenantId, It.IsAny<Guid>(), payload.DayOfWeekIndex, payload.StartTime, payload.EndTime, ScheduledClassId))
            .ReturnsAsync(false);
        scheduledClassDao
            .Setup(dao => dao.UpdateForTenantAsync(It.Is<ScheduledClassUpdate>(update => update.Id == ScheduledClassId), TenantId, transactionScope.Object))
            .ReturnsAsync(true);
