...
public class ClaimContextTests
{
    ...
    [Test]
    public void IndexCoreServicesPyramid_WhenClaimPresentAndValid_ReturnsParsedInteger()
    {
    ...
    {
        GivenClaims(new Claim("index_core_services_pyramid", "not-an-integer"));
        var claimContext = new ClaimContext(_httpContextAccessor.Object);

        MissingClaimException exception = Assert.Throws<MissingClaimException>(() => _ = claimContext.IndexCoreServicesPyramid)!;
        Assert.That(exception.ClaimName, Is.EqualTo("index_core_services_pyramid"));
    }
    ...
    public void SubscriptionExpiresAt_WhenClaimMalformed_ThrowsMissingClaimException()
    {
        ...
        MissingClaimException exception = Assert.Throws<MissingClaimException>(() => _ = claimContext.SubscriptionExpiresAt)!;
        Assert.That(exception.ClaimName, Is.EqualTo("subscription_expires_at"));
    }
}
