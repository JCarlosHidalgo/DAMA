...
El flujo de negocio sensible es el **pago QR de clases** en Payment: crear deuda, recibir el callback de la pasarela externa Todotix, y acreditar clases al estudiante. Está protegido en cuatro capas.

### 1. Rol + tier de suscripción (autorización de función + de nivel)

Cada acción del flujo QR exige ser `Student` **y** tener nivel de suscripción ≥ 3 vigente. Dos atributos en cascada, `[Authorize(Roles = UserRoles.Student)]` y `[RequiresServiceTier(3)]`, decoran la acción que crea la deuda (`apps/Payment/Backend/Controllers/QrPaymentController.cs:43`).
...
El punto de acceso de callback es `[AllowAnonymous]` (lo llama la pasarela, no un usuario), así que su autenticidad se valida con **HMAC-SHA256** sobre el `transaction_id`. Una firma inválida se descarta silenciosamente: la acción comprueba `_callbackSignature.Verify(...)` y, si la firma no coincide, registra el evento mediante `LogEvents.TodotixCallbackInvalidSignature` y responde `200` sin encolar nada; solo cuando la firma es válida invoca `_callbackInbox.TryEnqueueAsync(...)` (`apps/Payment/Backend/Controllers/QrPaymentController.cs:128`).

La verificación usa comparación en tiempo constante (`CryptographicOperations.FixedTimeEquals`) para evitar *timing attacks* (`apps/Payment/Backend/Services/Concrete/CallbackSignature.cs:38`).
...
### 3. Bandeja de Entrada idempotente (anti-replay del callback)

El callback no muta la deuda directamente: se **encola** en `payment_callback_inbox`. El encolado es idempotente — la PK es el `transaction_id`, y un duplicado se descarta: el DAO intenta el `INSERT` y, si MySQL devuelve el error `1062` (clave duplicada), lo captura y retorna `false` en vez de propagar la excepción (`apps/Payment/Backend/DB/Daos/Concrete/Single/QrPayments/PaymentCallbackInboxDao.cs:40`).

Reproducir el mismo callback (incluso con firma válida) no vuelve a transicionar la deuda: ya está en la Bandeja de Entrada. Esto cierra el abuso por *replay* del flujo de negocio sensible.

### 4. Re-consulta a Todotix antes de transicionar

La Bandeja de Entrada la drena un `BackgroundService` que, en vez de confiar en los parámetros del callback, re-consulta a Todotix y solo entonces transiciona. El worker resuelve el handler de comando por *scope* fresco, invoca `callbackHandler.Handle(new ProcessQrCallbackCommand(...))` y marca el registro como procesado con `callbackInboxDao.MarkProcessedAsync(...)` tras el efecto (`apps/Payment/Backend/Workers/QrPayments/PaymentCallbackWorker.cs:83`).
...
Cuando la deuda se captura, Payment emite `payment.captured` por Bandeja de Salida/RabbitMQ y Attendance acredita las clases bajo `processed_events` (ver A04 §1 y `PaymentCapturedHandler.cs:38`), de modo que ni el extremo de pago ni el de acreditación pueden duplicar el efecto.
...
Student (rol + tier ≥ 3)
   ...
   ▼
POST /api/payment/qr/{templateId}  → crea deuda (Bandeja de Salida Todotix)
   ⋮  el estudiante paga vía pasarela Todotix
   ▼
GET /api/payment/qr/callback  [AllowAnonymous]
   │  [Firma HMAC del callback]  Verify(transaction_id, sig)  ── inválida ──▶ 200, descarta
   │ válida
   ▼
[Bandeja de Entrada idempotente]  payment_callback_inbox  ── 1062 ──▶ replay descartado
   │
   ▼  PaymentCallbackWorker (BackgroundService)
re-consulta Todotix → ProcessQrCallbackCommand → transiciona deuda
   │
   ▼  payment.captured (Bandeja de Salida → RabbitMQ)
...
2. La **firma HMAC del callback** valida que el aviso de pago viene realmente de Todotix; firmas inválidas se descartan sin efecto.
3. La **Bandeja de Entrada idempotente** absorbe el callback y neutraliza replays; el worker re-consulta a Todotix antes de transicionar, y la acreditación final vuelve a ser idempotente en Attendance.
...
- Confirmar el encolado idempotente del inbox:
  ...
  ```
- Pruebas: `apps/Payment/Test/Security/RequiresServiceTierAttributeTests.cs` cubre el *gating* de nivel; la suite de Payment (186 pruebas) ejercita firma y procesamiento de callback.
...
- No hay CAPTCHA / *human detection* en la creación de deuda: el control de abuso descansa en rol + tier + limitación de tasa de peticiones de la puerta de enlace (A04/API4), no en detección de bots. Si el flujo se abriera a un tier más bajo, convendría reforzar con un límite por usuario específico del flujo.
