...
`apps/Attendance/Backend/Services/Concrete/Events/PaymentCapturedHandler.cs:38`
```csharp
return await IdempotentTransaction.RunAsync<PaymentCapturedOutcome>(
    ...
    async scope =>
    {
        await _remainClassesDao.IncrementAsync(/* … */, transaction: scope);
        await _paymentCreditLedgerDao.RecordAsync(paymentCapturedEvent.EventId, /* … */, scope);
...
`apps/Auth/Backend/DB/Daos/Concrete/Single/Users/UserDao.cs:124`
```csharp
...
`infrastructure/environments/auth/init.sql:133`
```sql
...
`apps/Auth/Backend/DB/Daos/Concrete/Single/Users/UserDao.cs:19`
```csharp
...
`apps/Payment/Backend/Security/RequiresServiceTierAttribute.cs:36`
```csharp
...
`apps/Attendance/Test/Architecture/OutboxPublishingRulesTests.cs:15`
```csharp
