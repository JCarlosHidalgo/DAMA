...
DAMA injects secrets as plain environment variables through `infrastructure/.env.dev` (dev) and `infrastructure/.env.prod` (Dokploy production). The files are gitignored — only `.env.example` (dev) and `.env.staging.example` / `.env.prod.example` (templates) are committed. There is no Vault, Docker Secrets, or Key Vault integration; rotation is manual and per-environment.
...
| File | Status | Purpose |
| --- | --- | --- |
| `.env.example` | committed | dev defaults, safe to share |
| `.env.staging.example` | committed | template for a staging cluster |
| `.env.prod.example` | committed | template for production |
| `.env.dev`, `.env.staging`, `.env.prod` | gitignored | the real values |
