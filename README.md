...
| `envs/<Service>/` | Per-service `appsettings*.json` / `dbsettings.json` (copied into the image at build time) |
| `infrastructure/` | Compose files, per-service Dockerfiles, DB init/seed, gateway, TLS bootstrap |
